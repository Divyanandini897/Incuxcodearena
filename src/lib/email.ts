...
export async function sendOtpEmail(
  ...
  // ⭐ FIX: Prioritize real SMTP (Gmail) credentials first over Resend!
  if (hasRealCredentials()) {
    try {
      const transporter = await createTransporter();
      ...
      console.log(`[EMAIL] OTP successfully sent to ${email} using Gmail SMTP.`);
      return { success: true };
    } catch (err: any) {
      console.error('[EMAIL] Gmail SMTP error:', err.message);
      return { success: false, error: err.message || 'Gmail SMTP delivery failed' };
    }
  }

  // Fallback to Resend only if Gmail SMTP is not configured
  if (resend) {
    const { data, error } = await resend.emails.send({
      from: FROM,
      to: email,
  ...
  // Double fallback to ethereal (test account) if absolutely nothing else is set up
  try {
    const transporter = await createTransporter();
    const FROM_EMAIL = process.env.FROM_EMAIL || 'noreply@codenode.app';
...
export async function sendWelcomeEmail(
  ...
  if (hasRealCredentials()) {
    try {
      ...
      console.log(`[EMAIL] Welcome email successfully sent to ${email} using Gmail SMTP.`);
      return { success: true };
    } catch (err: any) {
      console.error('[EMAIL] Gmail SMTP Welcome error:', err.message);
      return { success: false, error: err.message || 'Gmail SMTP delivery failed' };
    }
  }
