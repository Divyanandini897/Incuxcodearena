...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const username = user.user_metadata?.preferred_username || email.split('@')[0].toLowerCase().replace(/[^a-z0-9_-]/g, '');
    ...
    if (upsertError) {
      console.error('[GOOGLE-CALLBACK] Profile upsert error:', upsertError);
    }
